...
def figure(rate_wifi_my, rate_wifi_oth, wifiset_num):
    ...
    ax.set_xlabel(r'Radius $r_s \ (km)$', font2)
    ...
    lns2 = ax.plot(x, rate_wifi_oth, ls='-', color='blue', marker="^", markersize=10, label= r"$k$ in approach 1")
    ax.set_ylim(0, 1.0)
    x_ticks = np.arange(0.4, 1.3, 0.2)
    plt.xticks(x_ticks)
    # -----------------------------------------------------------------------
    # ********************用户密度，集合数量********************************
    '''fig = plt.figure(2)
    ...
    # ***********************
    ax2 = ax.twinx()
    lns3 = ax2.plot(x, wifiset_num, ls='--', color='forestgreen', marker="d", markersize=10, label= r"$CS$")
    lns = lns1 + lns2 +lns3
    labs = [l.get_label() for l in lns]
    ax.legend(lns, labs, loc = 0)
    ax2.set_ylabel(r"Number of connected sets $CS$", font1)
    ax2.set_ylim(20, 120)
    ax.grid()
    ax.margins(0)
    plt.show()
...
def rate_wifi(filename, N):
    ...
    return rate_wifi_my, rate_wifi_oth, wifiset_num
# The process is very difficult, so it's annotated in Chinese
# Press the green button in the gutter to run the script.
if __name__ == '__main__':
    N = 12 # N指信道总数
    filename = 'E:\\pythonprojects\\WiFi\\geo.DBF'
    rate_wifi_my, rate_wifi_oth, wifiset_num = rate_wifi(filename, N)
    fig, ax = plt.subplots()
    figure(rate_wifi_my, rate_wifi_oth, wifiset_num)
